...
namespace Gui
{
    class VerticalLUFSMeter : public Component, public Timer
    {
    public:
        VerticalLUFSMeter(std::function<float()>&& valueFunction,
        ...
        void paint(Graphics& g) override
        {
            ...
            const auto scaledY = jmap(valueSupplier(), -100.f, 0.f, 0.f, static_cast<float>(getHeight()));
            g.fillRect(bounds.removeFromBottom(scaledY));

            // Draw Meter Hold Bar into Meter
            g.setColour(Colours::red);
            int meterHoldBarHeight = getHeight() / 100;
            const auto scaledHold = jmap(meterHoldValueSupplier(), -100.f, 0.f, 0.f, static_cast<float>(getHeight()));
